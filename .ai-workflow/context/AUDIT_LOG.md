...
## 2026-07-24 — VS Code state → repo_path resolution (window picker auto-fill)

- No existing code reads VS Code state (`state.vscdb`/`storage.json`/`globalStorage`) — clean slate, nothing to consolidate.
- Path normalization stays centralized: `src/repo-path.js` (`normalizeRepoPath`/`canonicalRepoPath`/`repoPathsEqual`); mcp-server's `normalizePath()` remains its intentional standalone copy. The basename-extraction one-liners in main.js:2501 and index.js:3553 are intentional case-preserving display variants, not consolidation targets.
- `better-sqlite3` is confined to `db-sqlite.js` — and stays that way: the resolver reads `storage.json` (plain JSON, `windowsState` + `backupWorkspaces` hold currently-open windows with full URIs), so `state.vscdb` recents (near-empty on this machine anyway) aren't needed at all. No sqlite locking/ABI concerns.
- Per audit recommendation the resolver lives in `src/window-info.js` (IDE discovery already lives there: `listIDEWindows`, `parseVSCodeWindowTitle`, `isWSL`); `listIDEWindows()` now annotates each window with `path`/`pathAlternates`. `normalizeRepoPath()`'s existing `.code-workspace`-stripping handles workspace windows for free.
- Known minor: `parseVSCodeWindowTitle` is exported but only used in-file — left as-is (small, useful for future IDE detection).
