#!/bin/bash
# ensure-workflow.sh — Health check for HuminLoop AI dev workflow (Windows/Git Bash)
...
echo -e "${YELLOW}HuminLoop Workflow Health Check${NC}"
echo ""
...
if command -v docker >/dev/null 2>&1; then
  ...
  check "HuminLoop DB container running" \
    "docker ps --format '{{.Names}}' 2>/dev/null | grep -q 'huminloop-db'" \
    "cd $PROJECT_ROOT && docker compose up -d"
else
  echo -e "  ${DIM}-${NC} Docker not installed (SQLite fallback available)"
fi

# HuminLoop API reachable
check "HuminLoop API reachable (port 7277)" \
  "curl -s http://127.0.0.1:7277/api/health >/dev/null 2>&1" \
  "npm start  (launch HuminLoop first)"
